...
async def connect_and_enumerate(client: BleakClient, device_label: str):
    """Connect to a BLE device and print all GATT services."""
...
async def connect_by_address(address: str):
    """Connect directly to a BLE device by MAC address."""
    print(f"Connecting directly to {address}...")
    try:
        async with BleakClient(address, timeout=15.0) as client:
            await connect_and_enumerate(client, address)
    except Exception as e:
        print(f"Failed to connect to {address}: {e}")


async def scan_and_connect(device_name: str, scan_timeout: float = 10.0):
    """Scan for a BLE device by name, then connect."""
    ...
    async with BleakClient(device) as client:
        await connect_and_enumerate(client, device.name)
...
def main():
    ...
    parser.add_argument(
        ...
        help="Connect directly by BLE MAC address (skip name scan)"
    )
    parser.add_argument(
    ...
    if args.address:
        asyncio.run(connect_by_address(args.address))
    else:
